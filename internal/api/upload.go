...
func (h *UploadHandler) HandleComplete(w http.ResponseWriter, r *http.Request) {
	...
	// Complete in MinIO
	if err := h.storage.CompleteMultipartUpload(r.Context(), objectName, session.UploadID, req.Parts); err != nil {
	...
	file := &models.File{
		...
		Size:           int64(len(req.Parts)) * 5 * 1024 * 1024, // Rough size, maybe should be passed in?
